...
/**
 * @file classes/citation/job/external/openalex/Inbound.php
 *
...
namespace PKP\citation\job\external\openAlex;
...
class Inbound
{
    ...
    public function getWork(Citation $citation): Citation
    {
        ...
        foreach (Mapping::getWork() as $key => $value) {
            switch ($key) {
                case 'authors':
                    $authors = [];
                    foreach (Helpers::getValueFromArrayPath($response, $value) as $index => $author) {
    ...
    private function getAuthor(array $authorShip): array
    {
        ...
        foreach ($mapping as $key => $val) {
            if (is_array($val)) {
                $author[$key] = Helpers::getValueFromArrayPath($authorShip, $val);
            } else {
