...
/**
 * @file classes/citation/job/external/crossref/Inbound.php
 *
...
namespace PKP\citation\job\external\crossref;

use APP\facades\Repo;
use PKP\citation\Citation;
use PKP\citation\job\Helpers;
...
class Inbound
{
        ...
        foreach (Mapping::getWork() as $key => $value) {
            switch ($key) {
                case 'doi':
                    $citation->setData(
                        $key,
                        Doi::addPrefix(Helpers::getValueFromArrayPath($response, $value))
                    );
                    break;
                default:
                    if (is_array($value)) {
                        $citation->setData($key, Helpers::getValueFromArrayPath($response, $value));
                    } else {
    ...
    {
        $score = (float)Helpers::getValueFromArrayPath($response, ['message', 'items', 0, 'score']);
        if (empty($score) ||  $score < $this->scoreTreshold) {
            return false;
        }

        $title = Helpers::getValueFromArrayPath($response, ['message', 'items', 0, 'title', 0]);
        if(empty($title) || !str_contains(strtolower($rawCitation), strtolower($title))) {
            return false;
        }

        $authors = Helpers::getValueFromArrayPath($response, ['message', 'items', 0, 'author']);
